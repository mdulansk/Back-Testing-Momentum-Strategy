...
def optimize(price_data,parameter_data,performance_data):
    ...
    # since we can only supply one argument to the pool process :/
    ...
    if parameter_data.optimize_params == False:
        ...
        for param_to_optimize, val in params_to_optimize.items(): # Select a parameter to optimize by testing a range of values
            ...
            for curr_value in curr_param_values: #Iterate through all values for parameter to optimize
                ...
                # Left off here ... Need to add required arguments to StrategyMultiMomentum obj instantiation?
                
    print(f'Total parameter combos {len(parameter_data.parameter_combos)} for run {parameter_data.run_number}.', flush=True)
    ...
    strategy_obj_df.sort_values(by='performance', ascending=False, inplace=True) # Sort the df by performance measure in descending order
    best_strategy_obj = strategy_obj_df.iloc[0]["stategy_obj"]
    #print(f'\nstrategy_obj_df: {strategy_obj_df}')
