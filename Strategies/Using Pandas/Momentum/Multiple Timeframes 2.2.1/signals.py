...
def get_top_tickers(current_tickers,date,qty_long,qty_med,qty_short,rolling_returns):
    ...
    top_coarse = rolling_long.loc[date].nlargest(qty_long).index # Get the top performing n stocks from longest rolling average returns
    top_med = rolling_med.loc[date, top_coarse].nlargest(qty_med).index # Get the top n performing stocks from medium length rolling average returns
    top_fine = rolling_short.loc[date, top_med].nlargest(qty_short).index # Get the top n performing stocks from shortest rolling average returns
    top_tickers_and_returns = rolling_short.loc[date, top_med].nlargest(qty_short) # Returns a series
