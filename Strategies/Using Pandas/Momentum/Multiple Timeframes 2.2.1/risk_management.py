...
def fixed_stop(period_daily_ret, stop, stop_type='avg'):
    # NOTE: This function is out of date and will need to be updated similar to the trailing stop function.
...
def position_sizing_sma(signals_df, leverage):
    # This function will accept signal data df and leverage multiplier (1 = no leverage) as arguments and return percentage position size based on signals.
    # Rules: get crossover signals for benchmark, note cross, note if short sma is above and moving away from long sma, if so increase position size.
    pass
