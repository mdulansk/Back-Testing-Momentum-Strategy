...
class DataClassParameters:
    ...
    all_time_stats = concat_stats_files() # This function returns stats from previous backtests
    ...
    tested_parameter_combos = None # DF containing previously tested parameter combos
    range_divisor: int = 2 #10 We divide the range of numbers for a parameter by this to determine how many data points to test
    ...
    run_max: int = 3 #5 # How many times to run the optimization function
    ...
    qty_long: int = 25 #30 #3 Number of stocks to hold from longest rolling period
    qty_med: int = 20 #20 #2 Number of stocks to hold from medium rolling period
    qty_short: int = 5 #2 #1 Number of stocks to hold from shortest rolling period
    SMA_L: int = 139 # Days
    SMA_S: int = 9 #9 # Days
    ...
    all_parameters = ['rebal_int','stop_type', 'stop', 'qty_long', 'qty_med', 'qty_short', 'SMA_L', 'SMA_S', 'rolling_l', 'rolling_m', 'rolling_s']
    params_to_optimize = ['qty_long']
...
def main():
    ...
    if instrument == 'nasdaq100':
        parameter_data = DataClassParameters(qty_long=25,qty_med=20,qty_short=10,SMA_S=(9),SMA_L=(139),
                                         rolling_l=(34), rolling_m=(16),rolling_s=(8),stop=40,resample_period='M',rebal_int=3) # Monthly values
        #parameter_data = DataClassParameters(qty_long=25,qty_med=20,qty_short=10,SMA_S=(2),SMA_L=(139),
        ...
        parameter_data = DataClassParameters(qty_long=3,qty_med=2,qty_short=1,SMA_S=(10),SMA_L=(200),
    ...
    optimization.get_tested_parameters(parameter_data)
    all_stats_df = optimization.optimize(price_data,parameter_data,performance_data)
    if len(all_stats_df) == 0:
        print(f'No performance results, exiting. {all_stats_df}')
        exit()
    ...
    #print(f'best_returns_series:  {best_returns_series}')
