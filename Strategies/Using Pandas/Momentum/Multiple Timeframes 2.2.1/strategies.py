...
class StrategyMultiMomentum:
    ...
    def __init__(self, index_yearly_members, ticker_selections, price_data, start_date, end_date, 
                 resample_period, rebal_int, qty_long, qty_med, qty_short, 
        ...
        self.rebal_int = int(rebal_int)
        self.qty_long = int(qty_long)
        self.qty_med = int(qty_med)
        self.qty_short = int(qty_short)
    ...
    def strategy(self):
        ...
        #print(f'Running strategy function for {self}')
        ...
        for i in range(0, len(period_returns)-(self.rebal_int), self.rebal_int):
            ...
            if signals_df.loc[last_period_end_date]["uptrend"] == True:
                ...
                top_tickers = signals.get_top_tickers(current_tickers,last_period_end_date,self.qty_long,self.qty_med,self.qty_short,rolling_returns)
                ...
                if self.stop_enable != False:
                    ...
                    self.stop = 0 # Set this to 0 so that the stats data doesn't look like we were using a stop
        ...
        self.stats = {'rebal_int':self.rebal_int,'qty_long':self.qty_long,'qty_med':self.qty_med,
                      'qty_short':self.qty_short,'SMA_S':self.SMA_S,'SMA_L':self.SMA_L, 'stop':self.stop,'rolling_l':self.rolling_l,
