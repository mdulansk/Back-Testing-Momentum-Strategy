...
def trailing_stop(period_daily_ret, stop_loss, stop_type='avg'):
    ...
    # IMPORTANT: you need to subtract 1 from entire returns dataframe instead of just when calculating trailing stop 
    # return since we sometimes take original return
    stop_loss = round((stop_loss*.01),2) #Convert back to percentage since it was expressed as integer to be used with range function
    period_daily_ret = period_daily_ret-1
    if stop_type == 'avg':
        period_daily_ret = pd.DataFrame(period_daily_ret.mean(axis=1),columns=['avg_ret'])
    #print(f'Applying {stop_loss*10}% stop loss for {pd.to_datetime(period_daily_ret.iloc[0].name).date()} - {pd.to_datetime(period_daily_ret.iloc[-1].name).date()}',flush=True)
...
def optimize(price_data,parameter_data,performance_data):
    ...
    params_to_optimize: dict = {'benchmark_sma_short':parameter_data.benchmark_sma_short,'benchmark_sma_long':parameter_data.benchmark_sma_long,
...
def main():
    ...
    if instrument == 'nasdaq100':
        parameter_data = DataClassParameters(qty_long_period=30,qty_med_period=20,qty_short_period=2,benchmark_sma_short=(10),benchmark_sma_long=(200),
                                         rolling_period_long=(12), rolling_period_med=(6),rolling_period_short=(3),stop_loss=15)
        parameter_data.ticker_csv = f'{cached_data}/{instrument}_tickers.csv'
        parameter_data.tickers = get_nasdaq_tickers(parameter_data)
    elif instrument == 'ETF':
        parameter_data = DataClassParameters(qty_long_period=3,qty_med_period=2,qty_short_period=1,benchmark_sma_short=(10),benchmark_sma_long=(200),
                                         rolling_period_long=(12), rolling_period_med=(6),rolling_period_short=(3),stop_loss=30)
