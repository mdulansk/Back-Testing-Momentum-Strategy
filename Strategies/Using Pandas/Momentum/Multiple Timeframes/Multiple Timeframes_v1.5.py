...
#benchmark_sma_200 = pd.Series(round(benchmark_df['Adj_Close'].rolling(100).apply(np.mean),2),name='SMA200')
#benchmark_sma_50 = pd.Series(round(benchmark_df['Adj_Close'].rolling(50).apply(np.mean),2),name='SMA50')
#benchmark_sma_20 = pd.Series(round(benchmark_df['Adj_Close'].rolling(20).apply(np.mean),2),name='SMA20')
#benchmark_sma_10 = pd.Series(round(benchmark_df['Adj_Close'].rolling(10).apply(np.mean),2),name='SMA10')
#benchmark_df=benchmark_df.join(benchmark_sma_200)
if refresh_data == True:
    benchmark_df = pd.DataFrame(process_ticker(benchmark,'2000-01-01',end_date)['Adj_Close'])#, name = benchmark) # Grab more data than we need since we'll be losing some to indicator warmup
    benchmark_df.to_csv(f'{cached_data}/benchmark_df.csv')
else:
    benchmark_df = pd.read_csv(f'{cached_data}/benchmark_df.csv', index_col=['Date'], parse_dates=True)
benchmark_df.head(3)

# %%
def generate_signals(benchmark_df, SMA_short, SMA_long):
    signals_df = benchmark_df.copy()
...
#signals_df,num_crosses = generate_signals(benchmark_df,1,200) # Setting short-term period = 1 is the same as using actual price for cross signal
#signals_df.index[0]
#signals_df.head(10)

#benchmark_df.head()
#benchmark_returns.head(30)
benchmark_daily_returns = benchmark_df['Adj_Close'].pct_change()
...
if refresh_data == True:
    ...
    for ticker in tickers[:]:
        tickers_and_prices[ticker] = process_ticker(ticker,start_date,end_date)['Adj_Close']
...
# %%
daily_returns.head()
monthly_returns.iloc[10].nlargest(10) # Check out top returns for specific date
#monthly_returns.index[0]
#print(daily_returns.iloc[:10,:5])
#print(daily_returns.iloc[:10,:5].mean(axis=1))
#avg_returns = pd.DataFrame(daily_returns.iloc[:10,:5].mean(axis=1),columns=['avg_ret'])
#print((avg_returns))
...
# %%
#ret_12, ret_6, ret_3 = get_rolling_ret(monthly_returns,12), get_rolling_ret(monthly_returns,6), get_rolling_ret(monthly_returns,3)
#ret_12.head(20)
...
def get_top_tickers(date,top_n,rolling_returns):
    ...
    top_50 = rolling_long.loc[date].nlargest(top_n[0]).index # Get the top 50 from the 12 month rolling average returns
    top_30 = rolling_med.loc[date, top_50].nlargest(top_n[1]).index # From the top 50 ,get the top 30 from the 6 month rolling average returns
    top_10 = rolling_short.loc[date, top_30].nlargest(top_n[2]).index # From the top 30 ,get the top 10 from the 3 month rolling average returns
    ...
    for ticker in top_10:
        print(f'{ticker}   1 MO:  {round(monthly_returns.loc[date, ticker],2)}  3 MO: {round(rolling_short.loc[date, ticker],2)}')
    '''
    return top_10
...
def trailing_stop(period_daily_ret, stop_loss, stop_type='avg'):
    ...
    for stock_idx in range(len(period_daily_ret.columns)): #Iterate through each stock
        ...
        #print(f'\n{stock_idx}\t{ticker}\tInitial stop:  {trailing_stop}')
        for row in range(len(period_daily_ret)-0): # go through daily return for entire rolling lookback period
            curr_date = period_daily_ret.iloc[row].name
            period_ret = period_daily_ret.iloc[row,stock_idx]
            #print(f'{curr_date}\tPeriod return:  {period_ret}')
            ...
            if cum_ret > peak_cum_return:
                #print(f'{curr_date}\t Cum_ret ({cum_ret}) > {peak_cum_return} (peak_cum_return)\ttrailing_stop:  {peak_cum_return - stop_loss}')
                ...
                #print(f'{curr_date}\t{ticker}\tCumulative return ({cum_ret}) < stop loss level ({trailing_stop})  SELL!')
                period_daily_ret.iloc[row+1:,stock_idx] = 0 # Set values for stock to 0 from next day to end of period df
                break # Exit the loop since we're no longer in the stock
            else:
                #print(f'{curr_date}\t(cum_ret) {cum_ret} < {peak_cum_return} (peak_cum_return)\ttrailing_stop:  {trailing_stop}')
...
def optimize(price_data,parameter_data,performance_data):
    ...
    params_to_optimize = {'benchmark_sma_short':parameter_data.benchmark_sma_short_min_max,'benchmark_sma_long':parameter_data.benchmark_sma_long_min_max,
                          'rolling_period_long':parameter_data.rolling_period_long_min_max,'rolling_period_med':parameter_data.rolling_period_med_min_max,
                          'rolling_period_short':parameter_data.rolling_period_short_min_max,'stop_loss':parameter_data.stop_loss_min_max}
    all_parameters = (static_parameters | params_to_optimize) # Merge both dictionaries to combine all parameters    
    #for idx in range(len(params_to_optimize)):
    for param_to_optimize, val in params_to_optimize.items(): # Select a parameter to optimize by testing a range of values
        step = ceil((val[1]-val[0])/10)
    ...
    print(f'\nRun {parameter_data.run_number}: {len(parameter_data.parameter_combos)} parameter combos.', flush=True)
    print("\t".join("{}: {}".format(k, v) for k, v in all_parameters.items()), flush=True)
    # Now that we have all of our parameter combinations to test, run strategy function, append results to stats df, then note the parameter values.
    #stats = strategy(parameter_data.parameter_combos[0]) # Test single run
    #print(stats)
    ...
    all_stats_df.sort_values(by=performance_measure, ascending=False,inplace=True) # Sort by specified parameter
    all_stats_df.reset_index(drop=True,inplace=True) # Reset index after sort
    performance_data.all_stats_df = pd.concat([all_stats_df, performance_data.all_stats_df]) # Add current stats to total stats df
    performance_data.all_stats_df.sort_values(by=performance_measure, ascending=False,inplace=True) # Sort by specified parameter
    ...
    if parameter_data.run_number <= parameter_data.run_max:
        # Set new parameter ranges around the optimal values from the last run
        benchmark_sma_short_min_max = create_new_param_vals(all_stats_df.iloc[0]['SMA_S'], parameter_data.new_param_multiplier)
        benchmark_sma_long_min_max = create_new_param_vals(all_stats_df.iloc[0]['SMA_L'], parameter_data.new_param_multiplier)
        rolling_period_long_min_max = create_new_param_vals(all_stats_df.iloc[0]['rolling_l'], parameter_data.new_param_multiplier)
        rolling_period_med_min_max = create_new_param_vals(all_stats_df.iloc[0]['rolling_m'], parameter_data.new_param_multiplier)
        rolling_period_short_min_max = create_new_param_vals(all_stats_df.iloc[0]['rolling_s'], parameter_data.new_param_multiplier)
        stop_loss_min_max = create_new_param_vals(all_stats_df.iloc[0]['stop'], parameter_data.new_param_multiplier)
        parameter_data2 = DataClassParameters(run_number=parameter_data.run_number,benchmark_sma_short_min_max=benchmark_sma_short_min_max,benchmark_sma_long_min_max=benchmark_sma_long_min_max,rolling_period_long_min_max=rolling_period_long_min_max,
                                            rolling_period_med_min_max=rolling_period_med_min_max,rolling_period_short_min_max=rolling_period_short_min_max,stop_loss_min_max=stop_loss_min_max)
        #all_stats_df = optimize(price_data,parameter_data2,performance_data)
        optimize(price_data,parameter_data2,performance_data)
...
def plot_returns(return_series,benchmark_return_series):
    #portfolio_cum_returns = pd.Series(returns, index=monthly_returns.index[1:]).cumprod() # Convert returns to Series and set index
    #benchmark_cum_returns = pd.Series(benchmark_monthly_returns, index=monthly_returns.index[1:]).cumprod() # Convert returns to Series and set index based on monthly portfolio returns
    ...
    #benchmark_monthly_returns = benchmark_monthly_returns.loc[portfolio_returns.index[0]:portfolio_returns.index[-1]].cumprod() # Trim benchmark to match portfolio returns df length
    #print(portfolio_returns.index)
    #print(benchmark_monthly_returns.index)
    portfolio_returns_w_slippage = pd.Series([i - 0.01 for i in return_series], index=monthly_returns.index[1:]).cumprod() # Convert returns to Series and set index 
    #to match index from monthly_returns starting with second row, then take cumulative product of the series.
...
class DataClassParameters:
    ...
    run_number: int = None
    run_max: int = 8
    benchmark_sma_short_min_max: tuple = None
    benchmark_sma_long_min_max: tuple = None
    rolling_period_long_min_max: tuple = None
    rolling_period_med_min_max: tuple = None
    rolling_period_short_min_max: tuple = None
    stop_loss_min_max: tuple = None
    stop_type: str = 'indiv'
    ...
    parameter_combos: dict = None
    new_param_multiplier: float = .5
    processes: int = 16
...
class DataClassPriceData:
    ...
    stock_returns: pd.core.frame.DataFrame = None
...
def main():
    ...
    benchmark_return_series = pd.Series(benchmark_monthly_returns, index=monthly_returns.index[1:])-1 #Convert from list to Series with DateTime index from monthly returns df, then subtract 1 to get return %
    ...
    price_data = DataClassPriceData(benchmark_prices=benchmark_df,benchmark_returns=benchmark_return_series,stock_prices=daily_prices_df,stock_returns=monthly_returns)
    parameter_data1 = DataClassParameters(run_number=1,benchmark_sma_short_min_max=(6,16),benchmark_sma_long_min_max=(140,180),rolling_period_long_min_max=(10,15),
                                          rolling_period_med_min_max=(6,10),rolling_period_short_min_max=(3,6),stop_loss_min_max=(2,15))
    performance_data = DataClassPerformanceData()
    all_stats_df = optimize(price_data,parameter_data1,performance_data)
