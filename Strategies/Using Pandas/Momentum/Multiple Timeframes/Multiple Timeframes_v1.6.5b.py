...
def get_price_data(parameter_data):
    ...
    weekly_returns = daily_returns.resample('W').prod().fillna(0) # Need to subtract 1 to get actual % returns.
    monthly_returns = daily_returns.resample('M').prod().fillna(0) # Need to subtract 1 to get actual % returns.
...
def get_top_tickers(date,top_n,rolling_returns):
    ...
    rolling_long = rolling_returns[0].loc[:,parameter_data.current_tickers] # All rows, but only current Nasdaq tickers.
    rolling_med = rolling_returns[1].loc[:,parameter_data.current_tickers] # All rows, but only current Nasdaq tickers.
    rolling_short = rolling_returns[2].loc[:,parameter_data.current_tickers] # All rows, but only current Nasdaq tickers.
...
class DataClassParameters:
    ...
    run_max: int = 2 #8 #4 # How many times to run th eoptimization function
...
def get_benchmark_data(returns_df,parameter_data):
    if parameter_data.refresh_data == True:
        ...
        benchmark_daily_returns = benchmark_price_df['Adj_Close'].pct_change().fillna(0)
    else:
...
def main():
    ...
    if instrument == 'nasdaq100':
        parameter_data = DataClassParameters(qty_long_period=30,qty_med_period=20,qty_short_period=5,SMA_S=(10),SMA_L=(146),
    ...
    #best_parameters = all_stats_df.loc[0,['top_n','SMA_short','SMA_long','stop','rolling_l','rolling_m','rolling_s']]
    best_stats = all_stats_df.iloc[0]
    best_parameters = {'stop_type': 'indiv', 'qty_long_period': best_stats['top_n'][0], 'qty_med_period': best_stats['top_n'][1], 'qty_short_period': best_stats['top_n'][2], 'SMA_S': best_stats['SMA_S'], 'SMA_L': best_stats['SMA_L'], 'rolling_l': best_stats['rolling_l'], 'rolling_m': best_stats['rolling_m'], 'rolling_s': best_stats['rolling_s'], 'stop': best_stats['stop']}
    print(f'best_parameters: {best_parameters}')
    print(f'BEST Parameter Combos:\n{parameter_data.parameter_combos[0:5]}')
    
    strategy(parameter_data.parameter_combos[-1]) # Call the strategy function with the best optimized parameters so we can capture the returns from performance_data.return_series
    
    best_returns_series = performance_data.return_series
    ...
    print((f'{len(benchmark_return_series.index)} periods\t{benchmark_return_series.index[0]} - {benchmark_return_series.index[-1]}'))
    print((f'{len(best_returns_series.index)} periods\t{best_returns_series.index[0]} - {best_returns_series.index[-1]}'))
    ...
    #selections_df = pd.DataFrame(parameter_data.ticker_selections)
    selections_df = pd.DataFrame(parameter_data.ticker_selections.items(), columns=['Date','Tickers'])
    selections_df.to_csv(f'{cached_data}/selections.csv', index=False)
    print(selections_df.head())
    '''
