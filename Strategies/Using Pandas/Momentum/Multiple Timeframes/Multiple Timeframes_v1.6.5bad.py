...
def get_price_data(parameter_data):
    ...
    weekly_returns = daily_prices_df.resample('W').agg(lambda x: x[-1]).pct_change().fillna(0)
    monthly_returns = daily_prices_df.resample('M').agg(lambda x: x[-1]).pct_change().fillna(0)
    #print(f"monthly_prices:\n{daily_prices_df.resample('M').agg(lambda x: x[-1])}")
    #print(f'monthly_returns:\n{monthly_returns.head(11)}')
...
def get_rolling_ret(df, lookback):
    ...
    #return (df+1).rolling(lookback).apply(np.prod)
    rolling_ret = df.rolling(lookback).apply(np.sum)
    #print(f'rolling_ret for {lookback} period  lookback:\n{rolling_ret.head(11)}')
    return rolling_ret
...
def fixed_stop(period_daily_ret, stop, stop_type='avg'):
    ...
    return(curr_ret) # Add 1 to entire return so we take prod of return series
        ...
        if signals_df.loc[date]["uptrend"] == True:
            ...
            if parameter_data.stop_enable != False:
                if parameter_combos['stop'] != 0:        
                    ...
                    #print(f'{date}\tportfolio_month_ret: {portfolio_month_ret}')
                else:
...
class DataClassParameters:
    ...
    param_multiplier: float = 0 #1.5 #.8 #This is used to create range of parameter values for optimization. For example, specifying .8 means expanding value +/- 80%
    processes: int = 15#16
...
def get_benchmark_data(returns_df,parameter_data):
    if parameter_data.refresh_data == True:
        ...
        #benchmark_daily_returns = benchmark_price_df['Adj_Close'].pct_change() #Simple returns
        ...
        #benchmark_daily_returns = benchmark_price_df['Adj_Close'].pct_change()+1
        benchmark_daily_returns = (np.log(benchmark_price_df['Adj_Close']).diff()).fillna(0) #Log returns
    ...
    if parameter_data.synthetic_benchmark == True: # Take mean return of universe as benchmark
        ...
        print(f'Benchmark:  {parameter_data.benchmark_ticker}')
        ...
        benchmark_returns = (benchmark_daily_returns)[1:].resample(parameter_data.resample_period).sum()
        #print(f'benchmark_returns: {benchmark_returns[:5]}')

    print(f"Benchmark start price {benchmark_price_df.index[0].date()}: ${benchmark_price_df['Adj_Close'][0]},  end price {benchmark_price_df.index[-1].date()}: ${benchmark_price_df['Adj_Close'][-1]}")
    print(f"Benchmark Tot Manual Return {(benchmark_price_df['Adj_Close'][-1]/benchmark_price_df['Adj_Close'][0])-1}")
    print(f'Benchmark daily sum total return: {benchmark_daily_returns.sum()}')
    print(f'Benchmark monthly sum total return: {benchmark_returns.sum()}')
    print(f'Benchmark cumsum return: {benchmark_returns.cumsum()[-1]}')
    
    #print(f'Benchmark daily prod total return: {benchmark_daily_returns.prod()-1}')
    #print(f'Benchmark monthly prod total return: {benchmark_returns.prod()-1}')
    #print(f'Benchmark cumprod return: {benchmark_returns.cumprod()[-1]-1}')
...
def main():
    ...
    if instrument == 'nasdaq100':
        parameter_data = DataClassParameters(qty_long_period=30,qty_med_period=20,qty_short_period=5,SMA_S=(20),SMA_L=(200),
                                         rolling_l=(12), rolling_m=(7),rolling_s=(6),stop=25)
