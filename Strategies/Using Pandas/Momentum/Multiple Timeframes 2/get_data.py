...
def get_benchmark_data(returns_df,parameter_data):
    if parameter_data.refresh_data == True:
        benchmark_price_df = pd.DataFrame(StockPriceData.process_ticker(parameter_data.benchmark_ticker,parameter_data.benchmark_start_date,parameter_data.end_date)['Adj_Close'])#, name = benchmark) # Grab more data than we need since we'll be losing some to indicator warmup
...
def get_price_data(parameter_data):
    if parameter_data.refresh_data == True:
        ...
        for ticker in parameter_data.tickers[:]:
            tickers_and_prices[ticker] = StockPriceData.process_ticker(ticker,parameter_data.start_date,parameter_data.end_date)['Adj_Close']
    ...
    monthly_returns.to_csv(f'{parameter_data.cached_data}/nasdaq_monthly_returns.csv')
