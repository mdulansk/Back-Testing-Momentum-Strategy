...
def get_top_tickers(current_tickers,date,top_n,rolling_returns):
    ...
    top_tickers_and_returns = rolling_short.loc[date, top_med].nlargest(top_n[2])
    print(f'Top Tickers for {date}: {[(ticker, round(ret,2)) for ticker, ret in top_tickers_and_returns.iteritems()]}')
