...
def get_top_tickers(current_tickers,date,top_n,rolling_returns):
    ...
    print(f'\n{date}\tTop ticker returns:\t{round(top_tickers_and_returns.values.mean()-1,2)}\t{[(ticker, round(ret,2)) for ticker, ret in top_tickers_and_returns.iteritems()]}', flush=True)
