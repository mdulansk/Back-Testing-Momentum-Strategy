...
def set_best_parameters(parameter_data,performance_data):
    ...
    #print(f'parameter_data after setting to best:  {parameter_data}')
...
def optimize(price_data,parameter_data,performance_data):
    ...
    strategy_objs = []  # List to store strategy object references
    ...
    params_to_optimize = parameter_data.params_to_optimize.copy()
    static_parameters = parameter_data.static_parameters.copy()
    ...
    parameter_combos_df = (pd.DataFrame(parameter_data.parameter_combos, columns=parameter_data.parameter_combos[0].keys())) # Create dataframe from list of dictionaries
    ...
    print(f'Total parameter combos {len(parameter_data.parameter_combos)} for run {parameter_data.run_number}.\n', flush=True)
    ...
    #for parameter_combo in parameter_data.parameter_combos:
    ...
    strategy_obj_dict = dict([(strategy_obj, strategy_obj.performance) for strategy_obj in strategy_objs]) # Create a dictionary of strategy objects and assoc. perf
    #print(f'strategy_obj_dict: {strategy_obj_dict}')
    strategy_obj_df = (pd.DataFrame(list(strategy_obj_dict.items()), columns=['stategy_obj','performance'])) # Create dataframe from list of tuples from dictionary
    ...
    best_strategy_obj = strategy_obj_df.iloc[0]["stategy_obj"]
    print(f'\nstrategy_obj_df.head(10): {strategy_obj_df.head(10)}')
