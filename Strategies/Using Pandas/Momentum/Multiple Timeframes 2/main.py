...
class DataClassParameters:
    ...
    run_max: int = 8 #4 # How many times to run the optimization function
    ...
    SMA_L: int = 140 # Days
    SMA_S: int = 9 # Days
    rolling_l: int = 16 # Months (or weeks depending on value of resample_period)
    rolling_m: int = 11 # Months (or weeks depending on value of resample_period)
    rolling_s: int = 1 # Months (or weeks depending on value of resample_period)
    static_parameters = {'stop_type':stop_type,'stop':stop, 'qty_long_period':qty_long_period, 'qty_med_period':qty_med_period, 'qty_short_period':qty_short_period}
    params_to_optimize = {'SMA_L':SMA_L, 'SMA_S':SMA_S, 'rolling_l':rolling_l, 'rolling_m':rolling_m, 'rolling_s':rolling_s}
...
def main():
    ...
    best_returns_series = performance_data.best_return_series  # This doesn't actually give us the best returns, just the last returns when the sttrategy function was run.
    ...
    #print(f'best_returns_series:\n {best_returns_series.describe()}')
    #print(f'benchmark_return_series:\n {benchmark_return_series.describe()}')
