...
start_date = '2000-08-01' #Start date for data
end_date = '2022-08-01'
...
class ManyIndicatorStrategy(Strategy):
    ...
    def next(self):
        ...
        # TRAILING STOP
        self.current_buy_stop = max(self.current_buy_stop, (1-self.long_stop_pct*.01)*price) #Don't keep moving DOWN!
        self.current_sell_stop = min(self.current_sell_stop, (1+self.short_stop_pct*.01)*price) #Don't keep moving UP!
        #print(f'{current_date}   PRICE:  {round(price,2)}  STOPS:  Buy: {round(self.current_buy_stop,2)} ({round((1-self.current_buy_stop/price)*100,1)})%,  Sell: {round(self.current_sell_stop,2)} ({round((1-price/self.current_sell_stop)*100,1)}%)')
        if price < self.current_buy_stop and self.position.is_long:
            self.position.close()
            print(f'{current_date}   CLOSING Long position at ({round(price,2)}), dropped {round((1-self.current_buy_stop/price)*100,1)}% below stop ({round(self.current_buy_stop,2)})')
        if price > self.current_sell_stop and self.position.is_short:
            self.position.close()
            print(f'{current_date}   CLOSING Short position at ({round(price,2)}), rose {round((1-price/self.current_sell_stop)*100,1)}% above stop ({round(self.current_sell_stop,2)})')
        elif crossover(self.MA_short, self.MA_long): #BUY
        #self.buy()
            if short_only == False:
                #self.buy(sl=(1-self.long_stop_pct*.01)*price)
                self.buy()
                self.current_buy_stop = ((1-self.long_stop_pct*.01)*price) #Don't keep moving DOWN!
                print(f'{current_date}   BUY {ticker} at {price} with trailing stop at {self.current_buy_stop} ({round((1-self.current_buy_stop/price)*100,1)})% below price)')
            ...
            if long_only == False:
                ...
                self.sell()
                self.current_sell_stop = ((1+self.short_stop_pct*.01)*price) #Don't keep moving UP!
                print(f'{current_date}   SELL Short {ticker} at {price} with trailing stop at {round(self.current_sell_stop,2)} ({round((1-price/self.current_sell_stop)*100,1)}% above price)')
